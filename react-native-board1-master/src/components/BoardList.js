...
export default class BoardList extends React.Component{
  ...
  renderBoard({item}){
    ...
    return(
      ...
      <TouchableOpacity onPress={()=>this.props.deleteFunc(item.key)} style = {{flex : 1}}>
        <Text style = {{fontSize : 25, color : 'red'}}>X</Text>
      </TouchableOpacity>
      <TouchableOpacity onPress={() => this.props.navigation.push('Edit', {item:item, editFunc : this.props.editFunc})}>
        <Text style = {{fontSize : 15, color : 'blue'}}>수정</Text>
      </TouchableOpacity>
      </View>
    )
  }
